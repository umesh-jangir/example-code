...
function commercekit_admin_page_update() {
	...
	if ( isset( $_POST['commercekit'] ) ) {
		...
		if ( 'attribute-swatches' === $tab ) {
			...
			if ( isset( $_POST['clear_as_cache'] ) && 1 === (int) $_POST['clear_as_cache'] ) {
...
function commercekit_admin_page_html() {
	...
	<div class="wrap">
		...
		<nav class="nav-tab-wrapper" id="settings-tabs">
			...
			<a href="?page=commercekit&tab=pdp-attributes-gallery" data-tab="pdp-attributes-gallery" class="nav-item <?php echo 'pdp-attributes-gallery' === $tab ? 'nav-item-active' : ''; ?>"><?php esc_html_e( 'Product Attributes Gallery', 'commercegurus-commercekit' ); ?></a>
			...
			<?php if ( $domain_connected ) { ?>
				<a href="?page=commercekit&tab=support" data-tab="waitlist" class="nav-item <?php echo 'support' === $tab ? 'nav-item-active' : ''; ?>"><?php esc_html_e( 'Support', 'commercegurus-commercekit' ); ?></a>
			<?php } ?>
		</nav>
...
function commercekit_as_clear_all_cache() {
	...
	if ( $enabled ) {
		...
		if ( $building ) {
			return esc_html__( 'Attribute swatches cache is building in background. You can clear or start re-building when this background process has completed.', 'commercegurus-commercekit' );
		}
		...
		$action_ids = as_get_scheduled_actions( $args, 'ids' );
		if ( ! count( $action_ids ) ) {
			as_schedule_single_action( time(), 'commercekit_attribute_swatche_build_cache_remove', array(), 'commercekit' );
		}
...
function commercekit_ajax_get_as_build_status() {
	...
	$enabled        = isset( $options['attribute_swatches'] ) ? (int) $options['attribute_swatches'] : 0;
	$enabled_plp    = isset( $options['attribute_swatches_plp'] ) ? (int) $options['attribute_swatches_plp'] : 0;
	$build_done     = isset( $options['commercekit_as_scheduled_done'] ) ? gmdate( 'M j H:i:s', $options['commercekit_as_scheduled_done'] ) : '';
	$build_cancel   = isset( $options['commercekit_as_scheduled_cancel'] ) && 1 === (int) $options['commercekit_as_scheduled_cancel'] ? true : false;
	$as_scheduled   = isset( $options['commercekit_as_scheduled'] ) && 1 === (int) $options['commercekit_as_scheduled'] ? true : false;
	$ajax['status'] = 1 === $enabled && 1 === $enabled_plp ? $building : 0;
	...
	$alert_id = '';
	if ( $enabled && 0 < count( $action_ids ) ) {
		$alert_id       = 'event-processing';
		$ajax['status'] = 1;
	} elseif ( $enabled && 0 < count( $action_ids2 ) && 0 === count( $action_ids3 ) ) {
		$alert_id       = 'event-created';
		$ajax['status'] = 1;
	} elseif ( $enabled && 0 === count( $action_ids ) && 0 < count( $action_ids2 ) && 0 < count( $action_ids3 ) ) {
		$alert_id       = 'event-completed';
		$ajax['status'] = 0;
		if ( $build_cancel || 0 < count( $action_ids4 ) ) {
			$alert_id = 'event-cancel';
		} elseif ( '' === $build_done ) {
			$alert_id = '';
		}
	}

	$ajax['message'] = $c_total . ' / ' . $p_total;
